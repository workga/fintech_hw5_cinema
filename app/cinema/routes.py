...
def list_user_reviews(user_id: int, page: Page = Depends(pagination)) -> List[Review]:
...
def list_user_marks(user_id: int, page: Page = Depends(pagination)) -> List[Mark]:
...
def show_movie_stats(movie_id: int) -> Movie:
...
def list_movie_reviews(movie_id: int, page: Page = Depends(pagination)) -> List[Review]:
...
def create_movie_review(
    movie_id: int, review: ReviewCreate = Body(...), user: User = Depends(auth_user)
) -> Review:
...
def list_movie_marks(movie_id: int, page: Page = Depends(pagination)) -> List[Mark]:
...
def create_movie_mark(
    movie_id: int, mark: MarkCreate = Body(...), user: User = Depends(auth_user)
) -> Mark:
